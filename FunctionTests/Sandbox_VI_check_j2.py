...
env = Env(J=1, S=2, mu=array([2]), lmbda=array([3]), t=array([2]),
          ...
          gamma=1, D=50, e=1e-6, trace=False)
    ...
    for s in s_states:
        for x in x_states:
            for i in arange(J+1):
                state = np.sum(i * sizes_i[0] + x * sizes_i[1:J + 1] + s * sizes_i[J + 1:J * 2 + 1])
                pi = Pi[state]
                Pi[state] = Keep_Idle if (sum(x) or (i < J)) else Pi[state]
                w = W[state]
                for j in arange(J):
                    if (x[j] > 0) or (j == i):  # Class i waiting, arrival, or time passing
...
while not converged:  # Update each state.
    ...
    for s in arange(S):
        for x in arange(D + 1):
            W_[0, x, s] = r[0] - c[0] if x > gamma * t[0] else r[0]
            W_[1, x, s] = r[0] - c[0] if x > gamma * t[0] else r[0]
            for y in arange(x + 1):
                W_[0, x, s] += P_xy[0, x, y] * V[y, s+1]
                W_[1, x, s] += P_xy[0, x, y] * V[y, s+1]
            W_[0, x, s] = max(W_[0, x, s], V[min(D, x + 1), s])
            W_[1, x, s] = max(W_[1, x, s], V[x, s])
        W_[1, 0, s] = V[0, s]
    W_[0, 0:D, S] = V[1:D+1, S]
    W_[0, D, S] = V[D, S]
    W_[1, :, S] = V[:, S]
    if not np.allclose(W_, W):
        print("W incorrect")
    ...
    for s in arange(1, S+1):
        V_t_[0, s] = env.lmbda[0] * W[0, 0, s] + s * env.mu[0] * W[1, 0, s - 1] + (env.tau - env.lmbda[0] - s * env.mu[0]) * V[0, s]
        V_t_[1:D+1, s] = gamma * W[0, 1:D + 1, s] + s * env.mu[0] * W[1, 1:D + 1, s - 1] + (env.tau - gamma - s * env.mu[0]) * V[1:D + 1, s]

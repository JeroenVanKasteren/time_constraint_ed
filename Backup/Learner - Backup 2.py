...
class ValueIteration(Learner):
    ...
    def value_iteration(self):
        ...
        while True: # Update each state.
            ...
            # Stopping condition
            if env.convergence(self.V_t, self.V, self.counter):
                break
            # Save and Rescale the value function
            self.V = self.V_t - self.V_t[(0,0)*self.J]
            self.counter += 1
        env.timer(False, self.name)

    def value_iteration_old(self):
        """Docstring."""
        env = self.env; env.self.counter = 0
        env.timer(True, self.name)
        while True: # Update each state.
            iterator = env.setup_iterate(self.V)
            while not iterator.finished:
                multi_state = env.iterate(iterator)
                self.one_step_lookahead(multi_state)
                env.convergence_old(self.V_t[multi_state], self.V[multi_state])
                # Save and Rescale the value function
                self.V = self.V_t - self.V_t[(0,0)*self.J]
                # Stopping condition
                if env.converged_old():
                    break
        env.timer(False, self.name)

    def one_step_lookahead(self, multi_state):
        """Docstring."""
        ...
        for i in range(self.J):
            ...
            dummy -= g
            value += g * self.W(multi_state)
            if(s_i > 0):
                next_state = list(multi_state)
                next_state[i*2+1] -= 1
                value += s_i * mu[i] * self.W(tuple(next_state))
                dummy -= s_i*mu[i]
        value += dummy * self.V[multi_state]
        return value / self.tau

    def W(self, multi_state):
        """Return value."""
        J = self.J
        if(sum(multi_state[slice(1,J*2,2)]) == self.s):
            return self.V[multi_state] + \
                self.env.c_D(multi_state[slice(0,J*2,2)])
        w = np.full(self.J+1, np.inf)
        for i in range(self.J):
            x_i = multi_state[i*2]
            if(x_i > 0):  # If someone of class i waiting
                next_state = list(multi_state)
                next_state[i*2] = slice(x_i, None, -1)
                next_state[i*2+1] += 1  # s
                w[i] = self.env.c(x_i, i) + \
                    np.sum(self.P_xy[i, x_i, slice(x_i+1)] * \
                           self.V[tuple(next_state)])
            else:  # Else no one of class i to take into service
                w[i] = np.inf  # Make sure w[i] > self.V[multi_state]
        # do nothing
        w[self.J] = self.V[multi_state] + \
            self.env.c_D(multi_state[slice(0,J*2,2)])
        return np.min(w)
...
class PolicyIteration(Learner):
    ...
    def __init__(self, env, timeout):
        ...
        self.V = np.zeros(self.dim)  # V_{t-1}
        self.V_t = np.zeros(self.dim)  # V_{t}
        self.g = 0
        self.P_xy = self.env.P_xy
        self.timeout = timeout
        self.trace = env.trace; self.print_modulo = env.print_modulo
    ...
    def policy_iteration(self):
        """Policy Iteration."""
        if self.trace: print('Starting policy iteration.')
        if self.trace: start = timeit.default_timer()
        J = self.J
        self.counter = 0
        unstable = True; counter = 0
        while unstable:
            self.counter += 1
            counter += 1
            # Policy Evaluation
            inner_counter = 0
            while True:
                delta = 0; inner_counter += 1
                # Update each state.
                it = np.nditer(self.V, flags=['multi_index']); pointer = 0
                while not it.finished:
                    multi_state = it.multi_index; it.iternext()
                    if(pointer % self.print_modulo == 0):
                    ...
                    if(sum(states[:,1]) > self.s):
                        continue
                    self.V_t[multi_state]=self.policy_evaluation(multi_state)
                    delta = max(delta, np.abs(self.V_t[multi_state] - \
                                              self.V[multi_state]))
                    # Update the value function
                    self.V = self.V_t - self.V_t[(0,0)*J]
                if self.trace: print("delta: ", delta,
                     "Inner iterations: ", inner_counter)
                # Stopping condition
                if delta < self.env.epsilon:
                    break
                if(inner_counter > self.timeout):
            ...
            if(counter > self.timeout):
                if self.trace: print('Policy Iteration reached timeout')
                unstable = False

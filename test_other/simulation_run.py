...
# #           seed=42,  # args.job_id*args.array_id,
...
# def ospi(env, fil, i, x, v):
#     """One-step policy improvement.
#     i indicate which class just arrived, i = J if no class arrived.
#     """
#     eye = np.eye(env.J, dtype=int)
#     x = np.minimum(np.round(x / env.gamma, 0), env.D - 1).astype(int)
#     # for i in range(J):
#     #     x[i] = min(np.round(x[i] / gamma, 0), D - 1)
#     pi = env.J
#     x_next = x if i == env.J else x + eye[i]
#     v_sum = np.zeros(env.J)
#     w_max = 0
#     for j in range(env.J):
#         w_max += v[j, x_next[j]]
#         v_sum += v[j, x_next[j]]
#         v_sum[j] -= v[j, x_next[j]]
#     for j in range(env.J):  # Class to admit
#         if fil[j]:
#             w = env.r[j] - env.c[j] if x[j] > env.gamma * env.t[j] else env.r[j]
#             w += sum(env.p_xy[j, x[j], :x[j]+1] * (v_sum[j] + v[j, :x[j]+1]))
#             if w > w_max:
#                 pi = j
#                 w_max = w
#     return pi
